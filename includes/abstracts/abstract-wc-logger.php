 ...
 * @since      1.0.0
 *
 * @package    Max_Marine_Background_Processor/includes/abstracts
 */
 ...
 *
 * @package  Max_Marine\Background_Processor\Core\Abstracts\Max_Marine_Background_Processor_Abstract_WC_Logger
 * @author   David Baumwald <david@dream-encode.com>
 */
class Max_Marine_Background_Processor_Abstract_WC_Logger {
	...
	public static function log( $data, $level = 'debug' ) {
		...
		$log_level_setting = max_marine_background_processor_get_plugin_setting( 'log-level', 'off' );
		...
		if ( ! function_exists( 'wc_get_logger' ) ) {
			...
			error_log(
				sprintf(
					...
					__( '%1$s->%2$s depends on `wc_get_logger` which is bundled with the WooCommerce plugin.', 'max-marine-background-processor' ),
					__CLASS__,
					__METHOD__
				)
			);
